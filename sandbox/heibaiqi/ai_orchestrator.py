"""
AI编排器 - 协调多级AI工作流（黑白棋版，带完整日志）

改编自 /workspace/xiangqi/ai_orchestrator.py。编排逻辑棋类无关，
仅适配黑白棋棋盘摘要（8×8 矩阵 + 合法落子点）与黑白棋语义（black/white、disc、flip 原语）。
"""
import json
import re
import copy
...
请分析并输出JSON。"""
    ...
    def _handle_a1_hardcoded(
        ...
        黑白棋适配：
        - undo_move：撤销落子 + 撤销翻转（恢复被翻转棋子的 side + 删除新增的 disc）
        - set_winner：winner 可为 black/white/draw
        """
        instruction = action.get("instruction", {})
        action_name = instruction.get("action", "")
        ...
        if action_name == "undo_move":
            ...
            if winner == "draw":
                msg = intent.get("response_to_player", "已设置为平局")
            else:
                label = {"black": "黑方", "white": "白方", "red": "黑方"}.get(winner, winner)
                msg = intent.get("response_to_player", f"已设置{label}获胜")
            return {
                "success": True,
                "type": "applied",
                "message": msg,
...
3. 只输出JSON，不要输出其他内容"""
    ...
    async def _handle_action_b_with_log(
        ...
        # 棋子类型映射表（黑白棋仅有 disc；纠正国际象棋术语为 disc）
        type_mapping = {
            "pawn": "disc",
            "rook": "disc",
            "knight": "disc",
...
- board配置包括：width/height互换、regions 坐标旋转
...
- name 必须与新 type 对应：黑白棋标准棋子 disc 的 name 按阵营区分
...
5. 只输出JSON，不要输出其他内容"""
    ...
    async def _handle_action_c_with_log(
        ...
        if side == "white" or "pieces_white.json" in target_files:
            target_config_name = "pieces_white"
...
2. 如果用户要求修改某方的棋子（如"黑方的disc"），直接修改当前文件
3. 如果用户要求"可以移动到任意一格"，可以在moves中添加自由移动
4. 修改后必须确保规则与修改前不同
5. 如果规则未变化，校验层会检测到并触发重试
"""

        side_label = "白方" if target_config_name == "pieces_white" else "黑方"
...
5. 只输出JSON，不要输出其他内容"""
    ...
    async def _handle_action_cp_with_log(
        ...
        遵循灵活编码原则：AI基于 jump/ray/flip 三原语组合生成新棋子规则。
        ...
        # 黑白棋标准类型仅有 disc；自定义棋子 type 不得与之冲突
        predefined_types = {"disc"}

        side_label = "白方" if target_config_name == "pieces_white" else "黑方"
        config_filename = f"{target_config_name}.json"

        board_geo = board_state.get("board", {})
        board_w = int(board_geo.get("width", 8))
        board_h = int(board_geo.get("height", 8))
...
3. 移动规则必须基于 jump/ray/flip 三原语组合生成
4. 复合移动能力使用多个move定义
...
6. 只输出JSON对象，不要输出其他内容"""
        ...
        for cp in added_pieces:
            ...
            for move in cp_moves:
                kind = move.get("kind", "")
                if kind not in ("jump", "ray", "flip"):
                    validation_errors.append(f"新棋子 '{cp_type}' 的 move.kind '{kind}' 不是合法值（合法值: jump/ray/flip）")
        ...
        inst_board_w = int(new_board_state.get("board", {}).get("width", board_w))
        inst_board_h = int(new_board_state.get("board", {}).get("height", board_h))
        ...
        for inst in added_instances:
            ...
            if len(inst_position) != 2:
                ...
                x, y = inst_position
                if not (0 <= x < inst_board_w and 0 <= y < inst_board_h):
                    validation_errors.append(f"新棋子实例 '{inst.get('id')}' 的位置 [{x},{y}] 超出棋盘范围 (0-{inst_board_w-1}, 0-{inst_board_h-1})")
...
请仔细检查并修正上述错误，确保输出符合所有要求。"""
    ...
    async def _call_code_ai_with_validation(
        ...
        # 黑白棋 pieces 配置名（pieces_black/pieces_white）也纳入规则变化检查
        pieces_config_names = ("pieces", "pieces_black", "pieces_white", "pieces_red")
        ...
        for attempt in range(max_retries + 1):
            ...
            # 规则变化检查（C类规则修改：pieces / pieces_black / pieces_white / pieces_red）
            if rule_change_check and config_name in pieces_config_names:
