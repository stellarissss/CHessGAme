...
- board配置包括：width/height互换、regions区域坐标旋转（water/trap/den的cells数组）
...
- name必须与新type对应的emoji（红黑双方使用相同emoji）
  - elephant(象) → 🐘
  - lion(狮) → 🦁
  - tiger(虎) → 🐯
  - leopard(豹) → 🐆
...
6. 只输出JSON对象，不要输出其他内容"""
        ...
        for inst in added_instances:
            ...
            if len(inst_position) != 2:
                ...
                x, y = inst_position
                board_w = self.board_config.get("geometry", {}).get("width", 7)
                board_h = self.board_config.get("geometry", {}).get("height", 9)
                if not (0 <= x < board_w and 0 <= y < board_h):
                    validation_errors.append(f"新棋子实例 '{inst.get('id')}' 的位置 [{x},{y}] 超出棋盘范围 (0-{board_w-1}, 0-{board_h-1})")
