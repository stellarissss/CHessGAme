"""
机制引擎 v1.0 - 黑白棋游戏机制原语执行引擎
核心设计：机制原语硬编码实现，AI通过修改JSON配置来组合使用这些原语
（类比规则引擎的jump/ray/flip原子体系）
"""
import copy
import random
...
class MechanismEngine:
    ...
    def is_player_controlled(self, board_state: dict, side: str) -> bool:
        ...
        for item in pc_list:
            controlled_side = item.get("side", "black")
    ...
    def get_active_mechanisms_summary(self, board_state: dict) -> List[str]:
        ...
        for item in mech["skip_turns"]:
            side_label = self._side_label(item["side"])
        ...
        for item in mech["player_control"]:
            side_label = self._side_label(item.get("side", "black"))
    ...
    def apply_personality_to_ai(self, chess_ai_instance) -> None:
        """
        将性格配置应用到AI实例（OthelloAI）
        ...
        if depth_override is not None:
            chess_ai_instance.depth = max(1, int(depth_override))
        else:
            original_depth = chess_ai_instance.depth
            chess_ai_instance.depth = max(1, int(original_depth * preset["depth_mult"]))

        # 覆盖随机度
        randomness_override = personality.get("randomness_override")
        if randomness_override is not None:
            chess_ai_instance.randomness = max(0.0, min(1.0, float(randomness_override)))
        else:
            original_randomness = chess_ai_instance.randomness
            chess_ai_instance.randomness = max(0.0, min(1.0, original_randomness * preset["randomness_mult"]))

        # 存储性格参数到AI实例（供_evaluate使用）
        chess_ai_instance.personality_aggressiveness = agg
        chess_ai_instance.personality_conservatism = cons
        chess_ai_instance.personality_value_biases = personality.get("value_biases", {})
    ...
    def add_mechanism(self, board_state: dict, mechanism_type: str, side: str,
        ...
        Args:
            mechanism_type: skip_turns | ai_control | random_moves | extra_turns | move_limits | player_control
