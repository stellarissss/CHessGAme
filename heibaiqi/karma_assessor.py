...
### 黑白棋具体示例（必须参考）
- "让我的棋子可以跳一格翻转"：40 点（C 类 ×1.0）
- "创建一个能翻转十字方向的特殊棋"：80 点（C+ 类）
- "改棋盘颜色"：10 点（D 类）
- "让我的棋子可以斜着翻转"：35 点（C 类）
- "把棋盘从8×8改成10×10"：25 点（B 类）
- "让对手的棋子有30%概率不翻转"：50 点（A 类，高强度）
- "你好" / "讲个笑话"：1 点（E 类，固定）
...
class KarmaAssessor:
    ...
    def __init__(self, api_key: str = "", game_type: str = "heibaiqi"):
