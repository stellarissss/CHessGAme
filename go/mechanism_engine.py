...
class MechanismEngine:
    ...
    def get_active_mechanisms_summary(self, board_state: dict) -> List[str]:
        ...
        def get_side_label(side):
            return side_label_map.get(side, side)

        for item in mech["skip_turns"]:
            side_label = get_side_label(item["side"])
        ...
        for item in mech["player_control"]:
            side_label = get_side_label(item.get("side", "black"))
    ...
    def decrement_player_control(self, board_state: dict, side: str) -> None:
        ...
        当玩家代替AI走了一步棋后，消耗一次player_control机制。
        remaining为正数时递减，为0或负数时移除。
        """
        mech = self._get_mechanisms(board_state)
        pc_list = mech.get("player_control", [])
