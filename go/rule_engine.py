    ...
    def check_five_in_a_row(self, board_state: dict) -> Optional[str]:
        ...
        for x in range(width):
            for y in range(height):
                ...
                side = piece["side"]
                if self._check_line(board_state, x, y, 1, 0, side):
                    return side
                if self._check_line(board_state, x, y, 0, 1, side):
                    return side
                if self._check_line(board_state, x, y, 1, 1, side):
                    return side
                if self._check_line(board_state, x, y, 1, -1, side):
                    return side
