...
def validate_board_state(board_state: dict, board_config: dict = None) -> Tuple[bool, str]:
    """校验棋盘状态

    Args:
        board_state: 棋盘状态
        board_config: 棋盘配置（可选，用于获取棋盘尺寸；不传则使用默认19路）
    """
    ...
    errors = []
    if board_config:
        geometry = board_config.get("geometry", {})
