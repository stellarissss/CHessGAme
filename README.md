...
- 右下角常驻**小地图**：渲染大陆缩略图、玩家实时位置与各入口标点——未交互过的入口显示「❓」，已交互过的入口显示其对应 emoji。探索状态写入 `localStorage`（键 `chesssage_ow_explored`），关闭游戏后仍保留。
- 大陆南部沙漠设有「**沙盒训练场**」，以 iso-cube 聚合多面体（阶梯塔式演武平台 + 四角立柱）渲染；按 E 进入「纯净模式选棋类」（`/sandbox`）界面，返回时直接回到大陆。
...
**沙盒训练场**：南部沙漠 (40, 63) 的阶梯塔式演武平台——聚合多面体（砂岩基座 + 四角立柱 + 中央阶梯塔顶缀金灯）。按 E 进入纯净模式选棋类界面；该入口同样计入小地图探索。
