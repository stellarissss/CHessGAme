...
- **黑屏根治与自愈**：依据上述诊断修复三处真实 WGSL 编译错误（帧级 uniform 缺失 `@group(0) @binding(0)`、`fbm` 参数重赋值、`macro` 保留字变量——任一均使整套着色器编译失败→黑屏）；并内置设备丢失自动恢复——中途 `device.lost` 且非页面卸载/销毁时自动重载一次，界面不再永久卡黑。
