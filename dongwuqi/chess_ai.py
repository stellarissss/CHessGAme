"""
动物棋AI引擎 - 使用Minimax + Alpha-Beta剪枝
适配8动物等级体系 + 兽穴攻防 + check_win胜负判定
"""
import json
import re
import random
...
class AnimalChessAI:
    ...
    def _minimax(
        ...
        # 终止条件：动物棋胜负判定（进兽穴/全歼/困毙）
        winner = self.rule_engine.check_win(board_state)
        if winner:
    ...
    def _evaluate(self, board_state: dict, ai_side: str) -> float:
        ...
        for p in board_state.get("pieces", []):
            ...
            # 接近敌方兽穴奖励（激进性格更重视进攻兽穴）
            enemy_den = self._find_enemy_den_position(p["side"])
            if enemy_den:
                dx, dy = enemy_den
                dist = abs(px - dx) + abs(py - dy)
                max_dist = self.rule_engine._get_width() + self.rule_engine._get_height()
                if dist < max_dist:
                    base_bonus = 40
                    advance_bonus = base_bonus * (max_dist - dist) / max_dist * (0.5 + agg)
                    if p["side"] == ai_side:
                        score += advance_bonus
                    else:
                        score -= advance_bonus

            # 中心控制奖励（7×9棋盘中心约(3,4)，激进性格更重视）
            width = self.rule_engine._get_width()
            height = self.rule_engine._get_height()
            cx, cy = (width - 1) / 2, (height - 1) / 2
            base_center = ((width / 2) - abs(px - cx)) + ((height / 2) - abs(py - cy))
            if base_center > 0:
            ...
            # 保守性格：己方兽穴附近的棋子有额外防守加分
            if cons > 0.5:
                own_den = self._find_own_den_position(p["side"])
                if own_den:
                    gx, gy = own_den
                    dist = abs(px - gx) + abs(py - gy)
                    if dist <= 3:
                        defense_bonus = (4 - dist) * 15 * (cons - 0.5) * 2
                        if p["side"] == ai_side:
                            score += defense_bonus
                        else:
                            score -= defense_bonus
    ...
    def _find_enemy_den_position(self, side: str) -> Optional[List[int]]:
        """查找敌方兽穴位置"""
        enemy_den = "den_black" if side == "red" else "den_red"
        cells = self.rule_engine._region_cells.get(enemy_den)
        if cells:
            pos = next(iter(cells))
            return [pos[0], pos[1]]
        return None

    def _find_own_den_position(self, side: str) -> Optional[List[int]]:
        """查找己方兽穴位置"""
        own_den = "den_red" if side == "red" else "den_black"
        cells = self.rule_engine._region_cells.get(own_den)
        if cells:
            pos = next(iter(cells))
            return [pos[0], pos[1]]
        return None
    ...
    async def _evaluate_custom_piece_value(self, piece_rule: dict) -> int:
        ...
        movement = piece_rule.get("moves", [])
        rank = piece_rule.get("rank", 1)
        capture = piece_rule.get("capture", {})

        system_prompt = """你是动物棋（斗兽棋）棋子价值评估专家。根据棋子的等级、移动能力和吃子能力评估其价值。

参考价值表（标准动物）：
- 象(elephant): 800（rank8，最强，但被鼠克制）
- 狮(lion): 700（rank7，可跳河）
- 虎(tiger): 600（rank6，可跳河）
- 豹(leopard): 500（rank5）
- 狼(wolf): 400（rank4）
- 狗(dog): 300（rank3）
- 猫(cat): 200（rank2）
- 鼠(rat): 100（rank1，虽弱但克象且能入水）

评估依据：
1. 等级(rank)越高价值越高（基础价值约 rank×100）
2. 移动能力（jump四向/ray跳河/特殊移动）
3. 吃子能力（rank_ge默认 + exceptions例外如鼠克象）
4. 水域规则（能入水的鼠有特殊战略价值）
5. path_constraint跳河能力额外加成

输出要求：
- 只输出一个整数（50-1000范围），表示该棋子的价值
- 不要输出任何其他内容"""

        user_prompt = f"""请评估以下自定义动物棋子的价值：

棋子类型: {piece_type}
棋子名称: {piece_rule.get('label', '未知')}
等级(rank): {rank}
...
请评估这个棋子的价值（50-1000的整数），只输出数字。"""
    ...
    def _heuristic_custom_piece_value(self, piece_rule: dict) -> int:
        ...
        # 动物棋：基于 rank 估算价值
        rank = piece_rule.get("rank", 1)
        base_value = rank * 100  # rank1=100, rank8=800

        # 有跳河能力(ray + path_constraint)加成
        moves = piece_rule.get("moves", [])
        for m in moves:
            if m.get("kind") == "ray" and m.get("path_constraint"):
                base_value += 80
                break

        # 有例外吃子能力（如鼠克象）加成
        capture = piece_rule.get("capture", {})
        exceptions = capture.get("exceptions", [])
        if exceptions:
            base_value += 50

        # 有水域规则加成（如鼠入水）
        if capture.get("water_rules"):
            base_value += 30
