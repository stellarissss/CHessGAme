...
app = FastAPI(title="无限制跳棋", version="1.0.0")
    ...
    try:
        async with httpx.AsyncClient(timeout=5.0) as client:
            await client.post(
                ...
                json={"game_type": "tiaoqi", "event_type": event_type, "event_data": body}
            )
    except Exception:
        pass
    ...
    try:
        async with httpx.AsyncClient(timeout=5.0) as client:
            ...
            async with httpx.AsyncClient(timeout=5.0) as client2:
                ...
                await client2.post(f"{SAMSARA_API_URL}/api/turn/increment", json={"game_type": "tiaoqi"})
